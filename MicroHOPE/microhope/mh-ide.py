...
def setup_mh_bootloader():
	show('Setting up MicroHOPE bootloader via USBASP....')
	cmd = 'avrdude -B10 -c usbasp -patmega32 -U flash:w:ATmegaBOOT_168_atmega32.hex'
	print cmd
	res = commands.getstatusoutput(cmd)
	if res[0] != 0:
		show('Error: Check Connections....', 'red')
		return
	cmd = 'avrdude -B10 -c usbasp -patmega32 -U lfuse:w:0xff:m -U hfuse:w:0xda:m'
	res = commands.getstatusoutput(cmd)
	if res[0] != 0:
		show('Error: Setting up fuses', 'red')
		return
	mw.insert(END, res[1])	
	show('Upload Completed')
	
def	upload_usbast_to_mega8():
	show('Programming ATmega8 with USBASP....')
	cmd= 'avrdude -B10 -c usbasp -patmega8 -U flash:w:usbasp.atmega8.2011-05-28.hex'
	print cmd
	res = commands.getstatusoutput(cmd)
	if res[0] != 0:
		show('Error: Check Connections....', 'red')
		return
	cmd = 'avrdude -B10 -c usbasp -patmega8 -U lfuse:w:0xef:m -U hfuse:w:0xd9:m'
	res = commands.getstatusoutput(cmd)
	if res[0] != 0:
		show('Error: Setting up fuses', 'red')
		return
...
filemenu.add_command(label="Setup MicroHOPE bootloader", command = setup_mh_bootloader)
menubar.add_cascade(label="File", menu=filemenu,font=('Monospace', 12))
filemenu.add_command(label="Burn Mega8 with USBASP bootloader", command = upload_usbast_to_mega8)
